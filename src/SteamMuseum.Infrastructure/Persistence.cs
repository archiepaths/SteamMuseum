...
public sealed class MuseumDbContext(DbContextOptions<MuseumDbContext> options) : IdentityDbContext<MuseumUser, IdentityRole<Guid>, Guid>(options)
{
    ...
    protected override void OnModelCreating(ModelBuilder b)
    {
        ...
        b.Entity<Competence>().Property(x => x.Evidence).HasMaxLength(2000);
        b.Entity<Competence>().Property(x => x.RevocationReason).HasMaxLength(500);
        b.Entity<Competence>().HasIndex(x => new { x.MemberId, x.Role, x.RailwayId });
        b.Entity<Competence>().HasOne<Member>().WithMany().HasForeignKey(x => x.MemberId).OnDelete(DeleteBehavior.Restrict);
        b.Entity<Competence>().HasOne<Member>().WithMany().HasForeignKey(x => x.AssessedBy).OnDelete(DeleteBehavior.Restrict);
        b.Entity<Competence>().HasOne<Railway>().WithMany().HasForeignKey(x => x.RailwayId).OnDelete(DeleteBehavior.Restrict);
        b.Entity<Competence>().HasOne<Locomotive>().WithMany().HasForeignKey(x => x.LocomotiveId).OnDelete(DeleteBehavior.Restrict);

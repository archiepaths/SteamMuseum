...
public sealed class ApiFactory : WebApplicationFactory<Program>
{
    ...
    protected override void Dispose(bool disposing)
    {
        ...
        if (disposing) { if (File.Exists(database)) File.Delete(database); }
    }
}
...
public sealed class ApiTests : IAsyncLifetime
{
    ...
    public async Task Authentication_authorization_and_csrf_are_enforced()
    {
        ...
        Assert.Equal(HttpStatusCode.Forbidden, (await client.PostAsJsonAsync("/api/element-assessments", new { })).StatusCode);
        Assert.Equal(HttpStatusCode.Forbidden, (await client.PostAsJsonAsync("/api/accounts", new { })).StatusCode);
        Assert.Equal(HttpStatusCode.OK, (await client.GetAsync("/api/me/element-assessments")).StatusCode);
    }
    [Fact]
    public async Task Changing_roles_revokes_old_sessions_and_restricts_new_ones()
    {
        ...
        Assert.Equal(HttpStatusCode.Forbidden, (await member.PostAsJsonAsync("/api/element-assessments", new { })).StatusCode);
    ...
    public async Task Availability_roster_and_competence_workflow_enforces_rules()
    {
        ...
        var element = (await Success(await admin.PostAsJsonAsync("/api/competence-elements", new { name = "Driving", description = "Practical", learningType = "Practical", reassessmentMonths = 120 }))).GetProperty("id").GetGuid();
        var roleId = (await Success(await admin.PostAsJsonAsync("/api/competence-roles", new { name = "Driver", category = "Driver", railwayId = railway, locomotiveId = loco, elementIds = new[] { element } }))).GetProperty("id").GetGuid();
        var competence = (await Success(await admin.PostAsJsonAsync("/api/element-assessments", new { memberId = factory.MemberId, elementId = element, outcome = "Competent", assessedOn = DateOnly.FromDateTime(DateTime.UtcNow), evidence = "Practical assessment passed" }))).GetProperty("id").GetGuid();
        var days = new[] {
        ...
        async Task<Guid> Duty(string date, string start = "10:00:00", string end = "15:00:00") =>
            (await Success(await admin.PostAsJsonAsync("/api/duties", new { name = "Driver turn", date, start, end, role = "Driver", railwayId = railway, locomotiveId = loco, competenceRoleId = roleId }))).GetProperty("id").GetGuid();
        ...
        var history = await member.GetFromJsonAsync<JsonElement>("/api/me/element-assessments");
